...
2.  **Vous apprendre à faire des graphiques de qualités** dans `RStudio` et **vous faire prendre conscience de l'importance des visualisations graphiques** (chapitre @sec-viz : attention, ce chapitre est très long, ne vous laissez pas surprendre ! Et @sec-vizincert pour les statistiques descriptives). Les graphiques sont un outil essentiel pour l'analyse de données :
