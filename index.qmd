...
opts_chunk$set(
  ...
  cache.path = 'cache/',
  dev = if (
    knitr::is_html_output() && requireNamespace("ragg", quietly = TRUE)
  ) {
    "ragg_png"
  } else {
    "png"
...
Dans sa version complète, ce document représente un volume important, ce qui veut dire que vous devriez vous approprier **environ un chapitre par jour**, en comptant les week-ends ! Si vous n'avancez pas une journée, ce sont deux chapitres qu'il faudra intégrer le jour suivant... En particulier, le chapitre 3, consacré aux représentations graphiques, est très long et il est facile de se laisser dépasser.
