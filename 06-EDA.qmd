...
#| error: true
penguins |>
  summarise(Indices = quantile(bill_length_mm, na.rm = TRUE), .by = species)
```
...
```{r}
penguins |>
  reframe(Indices = quantile(bill_length_mm, na.rm = TRUE), .by = species)
```
...
```{r}
penguins |>
  reframe(enframe(quantile(bill_length_mm, na.rm = TRUE)), .by = species)
```
...
tmp |>
  ...
  geom_histogram(bins = 25, fill = "steelblue", color = "grey20", alpha = 0.7) +
  geom_vline(
    xintercept = mean(tmp$a, na.rm = TRUE),
    color = "firebrick",
    size = 1.5
  ) +
  geom_vline(
    xintercept = median(tmp$a, na.rm = TRUE),
    color = "orange",
    size = 1.5
  ) +
  labs(x = "Masse (g)", y = "Nombre") +
...
Le dernier bloc renseigne sur les variables numériques. Pour chacune d'elle, on a :
...
penguins |> # Avec le tableau penguins...
  select(species, bill_length_mm, bill_depth_mm) |> # Je sélectionne les variables d'intérêt...
