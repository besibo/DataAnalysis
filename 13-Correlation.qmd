...
ex1 <- tibble(x = 1:100, y = 1:100, z = 2 * y)
...
set.seed(12345)
ex2 <- tibble(x = 1:100, y = 1:100, z = 2 * y + rnorm(100, 5, 15))
...
#| warning: false
set.seed(12345)
ex3 <- tibble(x = 1:100, y = -(x - 50)^2 + 2500)
...
#| warning: false
set.seed(12346)
ex4 <- tibble(x = rnorm(100, 50, 10), y = rnorm(100, 50, 10))
...
birds |>
  ...
  geom_point() +
  labs(
    x = "Nombre de visites reçues par le poussin",
...
birds |>
  ...
  geom_rug() +
  labs(x = "Visites reçues par le poussin", y = "Densité")

birds |>
  ggplot(aes(x = futureBehavior)) +
  geom_density(fill = "firebrick2", alpha = 0.5) +
  geom_rug() +
  labs(x = "Agressivité à l'âge adulte", y = "Densité")
```
...
```{r}
cor.test(birds$nVisitsNestling, birds$futureBehavior, alternative = "greater")
```
...
cor.test(rope$impressiveness, rope$years, method = "spearman")
```
