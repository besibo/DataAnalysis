...
Light |>
  ggplot(aes(x = treatment, y = shift, fill = treatment)) +
  geom_boxplot(show.legend = FALSE, alpha = 0.3, outlier.colour = NA) +
  geom_jitter(width = 0.2, shape = 21, show.legend = FALSE) +
  labs(x = "traitement", y = "Décalage de phase (sans unité)") +
  scale_fill_brewer(palette = "Accent")
```
...
set.seed(1654)
dat <- tibble(
  num = c(rnorm(8, 8, 2), rnorm(8, 12, 6), rnorm(8, 16, 10), rnorm(8, 19, 14)),
  fac = rep(letters[1:4], each = 8)
)
out <- lm(num ~ fac, data = dat)

dat2 <- tibble(
  num = c(rnorm(8, 8, 14), rnorm(8, 12, 9), rnorm(8, 16, 6), rnorm(8, 19, 1)),
  fac = rep(letters[1:4], each = 8)
)
...
dat3 <- tibble(
  num = c(rnorm(8, 8, 20), rnorm(8, 12, 5), rnorm(8, 16, 5), rnorm(8, 19, 20)),
  fac = rep(letters[1:4], each = 8)
)
...
tidy(post_hoc2) |>
  ...
  labs(x = "Comparaison", y = "Différence de moyennes (et IC 95%)") +
  coord_flip() +
  theme_bw()
```

Notez ici l'utilisation de `geom_linerange`, pour afficher les intervalles de confiance à 95% des différences de moyennes. Il s'agit d'une alternative à `geom_errorbar()` dont nous avons déjà parlé à la @sec-errorbar. La fonction `geom_hline()` permet de faire apparaître des lignes horizontales sur un graphique. Ici, avec `y = 0`, cette fonction fait apparaître un axe horizontal (axe des abscisses). Enfin, la fonction `coord_flip()` permet d'inverser les axes du graphique : l'axe des `x` bascule à la verticale, et l'axe des `y` à l'horizontale. Cela permet d'obtenir un graphique dont l'apparence est typique de ce genre de graphique produit avec les résultats du test de Tukey HSD.
...
La syntaxe `_` est un raccourci qui permet de désigner l'objet transmis par le pipe `|>`.  Ici, cela signifie que nous utilisons les données du tableau `Light` filtrées grâce à la fonction `filter()` pour réaliser le test de Wilcoxon. Cela évite de devoir réécrire `Light` dans la fonction `wilcox.test()`.

Attention toutefois, cette syntaxe ne fonctionne que si le pipe **natif** est utilisé (ce pipe : `|>`). Si vous utilisez le pipe du tidyverse (`%>%`), vous ne pouvez pas utiliser `_` de cette manière : il faut utiliser le point à la place.
:::
