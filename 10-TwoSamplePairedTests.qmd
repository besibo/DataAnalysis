...
Si vous êtes dans une nouvelle session de travail (ou que vous avez quitté puis relancé `RStudio`), vous devrez penser à recharger en mémoire les packages utiles. Dans ce chapitre, vous aurez besoin d'utiliser les packages suivants :
...
- `rstatix` [@R-rstatix], qui permet de réaliser des tests statistiques avec des syntaxes compatibles avec le `tidyverse` et le `pipe`.
...
Si on ne dispose que d'un tableau au format large, on peut passer au format long, comme nous venons de le faire, grâce à la fonction `pivot_longer()`. Et si on ne dispose que d'un tableau au format long, on peut passer au format large grâce à la fonction `pivot_wider()`. Nous avons déjà décrit cette fonction en détail à la @sec-spread, et nous avons aussi vu plusieurs exemples de son utilisation notamment pour mettre en forme des résultats obtenus avec les fonctions `summarise()` (@sec-summarise) ou `reframe()` (@sec-reframe). Je vous encourage à y jeter un œil à nouveau pour vous remémorer la syntaxe, car il est important que vous maîtrisiez ces 2 fonctions dont vous aurez très souvent besoin.
...
Le package `rstatix` permet de réaliser tous ces tests avec une syntaxe compatible avec le pipe et la philosophie du tidyverse.
...
Testo_long |>
  t_test(DO ~ Moment, paired = TRUE, detailed = TRUE)

Testo_long |>
  wilcox_test(DO ~ Moment, paired = TRUE, detailed = TRUE)
```

Plusieurs choses importantes sont à noter ici :

1. le nom des fonctions contient des `_` plutôt que des `.`
2. le premier argument de toutes ces fonctions est toujours le nom d'un tableau. C'est la raison pour laquelle il est possible d'utiliser la syntaxe qui repose sur le pipe (`|>`)
3. les fonctions `t_test()` et `wilcox_test()` utilisent nécessairement un tableau au format long. Il est donc impossible d'utiliser la syntaxe qui repose sur la colonne `Difference` du tableau `Testo_large`.
4. les fonctions `t_test()` et `wilcox_test()` utilisent nécessairement la notation `formule`. Avec ce type de syntaxe, on place la variable expliquée à gauche du `~` et la variable explicative à droite. Ici, la variable expliquée est `DO` et la variable explicative est `Moment`. C'est la raison pour laquelle on utilise `DO ~ Moment` dans ces fonctions. Le `~` se lit toujours "en fonction de". On essaie don ici de comparer les différences de densité optique en fonction du moment de la mesure.
5. l'argument `paired = TRUE` est indispensable pour réaliser un test sur données appariées. Sans cet argument, les tests réalisés seraient des tests sur données indépendantes, ce qui ne serait pas correct. De ce point de vue, il n'y a pas de différences par rapport à l'utilisation des fonctions classiques `t.test()` et `wilcox.test()`.
6. l'argument `detailed = TRUE` permet d'obtenir les informations supplémentaires utiles : l'estimation de la moyenne des différences et les bornes de son intervalle de confiance.
7. les résultats sont fournis dans un tableau plutôt que sous forme de liste hétérogène. C'est donc plus lisible, et lorsque nous ferons des tests multiples, cela permettra de stocker les résultats dans un objet unique.

Au final, à vous de choisir si vous préférer utiliser les fonction classiques ou les fonctions du package `rstatix`. Elles fournissent strictement les mêmes résultats, et il s'agit donc d'une question de préférence uniquement.
