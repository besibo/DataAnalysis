...
gapminder_2007 |> 
  ggplot(aes(x = `GDP per Capita`, y = `Life Expectancy`, fill = Continent, 
             size = Population)) + 
...
penguins |>
  count(species, sex) |>
  mutate(cat = paste(species, sex, sep = " - ")) |> 
  ggplot(aes(y = fct_reorder(cat, n, .desc = TRUE), x = n, fill = fct_reorder(cat, n, .desc = TRUE))) +
...
penguins |> 
  ...
  scale_fill_brewer(palette = "Accent", labels = c("Adélie", "Chinstrap", "Gentoo")) +
  theme_bw() +
...
penguins |> 
  ...
  scale_fill_manual(labels = c("Femelles", "Mâles"), values = c("purple", "orange")) +
  theme_bw() +
  labs(x = "Longueur du bec (mm)", 
       y = "Épaisseur du bec (mm)",
       fill = "Sexe",
...
pp <- ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm,
                     fill = body_mass_g, size = body_mass_g)) +
  geom_point(shape = 21, color = "black", alpha = 0.6) +
  labs(x = "Longueur du bec (mm)",
       y = "Épaisseur du bec (mm)",
...
p1 <- pp + scale_fill_viridis_c(option = "A") + ggtitle('option = "A" (ou option = "magma")')
p2 <- pp + scale_fill_viridis_c(option = "B") + ggtitle('option = "B" (ou option = "inferno")')
p3 <- pp + scale_fill_viridis_c(option = "C") + ggtitle('option = "C" (ou option = "plasma")')
p4 <- pp + scale_fill_viridis_c(option = "D") + ggtitle('option = "D" (ou option = "viridis")')
p5 <- pp + scale_fill_viridis_c(option = "E") + ggtitle('option = "E" (ou option = "cividis")')
...
pplot1 <- ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm,
                     fill = species, size = body_mass_g)) +
  geom_point(shape = 21, color = "black", alpha = 0.6) +
  labs(title = "Forme du bec de 3 espèces de manchots et relation avec leur masse",
       subtitle = "Les manchots les plus lourds ont des becs longs et fins",
...
ggsave(filename = "images/fonts1.pdf", plot = pplot1, device = cairo_pdf, width = 7, height = 5)
```
...
#| message: false
#| warning: false
pplot2 <- ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm,
                     fill = species, size = body_mass_g)) +
  geom_point(shape = 21, color = "black", alpha = 0.6) +
  labs(title = "Forme du bec des manchots et relation avec leur masse",
       subtitle = "Les manchots les plus lourds ont des becs longs et fins",
...
ggsave(filename = "images/fonts2.pdf", plot = pplot2, device = cairo_pdf, width = 7, height = 5)
```
