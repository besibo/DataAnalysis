...
Dans le @sec-basics et le @sec-dataset, vous avez découvert les concepts essentiels qu'il est important de maîtriser avant de commencer à explorer en détail des données dans `R`. Les éléments de syntaxe abordés dans la @sec-code sont nombreux et vous n'avez probablement pas tout retenu. C'est pourquoi je vous conseille de garder le @#sec-basics à portée de main afin de pouvoir refaire les parties que vous maîtrisez le moins. Ce n'est qu'en répétant plusieurs fois ces tutoriels que les choses seront vraiment comprises et que vous les retiendrez. Ainsi, si des éléments de code présentés ci-dessous vous semblent obscurs, revenez en arrière : toutes les réponses à vos questions se trouvent probablement dans les chapitres précédents.
...
1. faire différents types de graphiques exploratoires avec le package `ggplot2` {{< fa chart-line >}} {{< fa chart-bar >}} {{< fa chart-area >}}
...
gapminder_2007 |>
  head() |>
  knitr::kable(
    caption = "Les 6 premières lignes du jeu de données `gapminder` pour l'année 2007.",
    booktabs = TRUE,
    longtable = TRUE
  )
```
...
Examinons maintenant la @fig-gapminder qui représente ces variables pour chacun des 142 pays de ce jeu de données (notez l'utilisation de la notation scientifique dans la légende, et de l'échelle logarithmique de l'axe des abscisses).
...
gapminder_2007 |>
  ...
  geom_point(color = "grey35", shape = 21) +
...
Comme indiqué plus haut, le package `ggplot2` [@R-ggplot2] permet de réaliser des graphiques dans `R` en respectant les principes de la grammaire des graphiques. Vous avez probablement remarqué que depuis le début de la section @sec-gggraph, beaucoup de termes sont écrits dans la police réservée au `code` informatique. C'est parce que les éléments de la grammaire des graphiques sont tous précisés dans la fonction `ggplot()` qui demande, au grand minimum, que les éléments suivants soient spécifiés :
...
penguins
```
Comme évoqué plus haut, il s'agit d'un `tibble`. Plusieurs de ses variables concernent la biométrie des manchots, en particulier de son bec (voir @fig-morpho).

![Morphométrie du bec des manchots. Illustration de Allison Horst](images/culmen_depth.png){#fig-morpho width=50%}
...
3. l'association de certaines variables du jeu de données (ici, `body_mass_g` et `bill_length_mm`) à certaines caractéristiques esthétiques du graphiques (ici, la position sur les axes des `x` et des `y`), grâce à l'argument `mapping` et la fonction `aes()`.
...
```{r}
#| fig-cap: "Relation entre masse corporelle et longueur du bec chez les manchots de l'archipel de Palmer"
ggplot(data = penguins, mapping = aes(x = body_mass_g, y = bill_length_mm)) +
  geom_point()
```
...
- Au sein de la fonction `ggplot()`, on spécifie 2 composants de la grammaire des graphiques :
    ...
    2. l'association (`mapping`) des variables du tableau de données à des caractéristiques esthétiques (`aes()`) en précisant `aes(x = body_mass_g, y = bill_length_mm)` :
...
- On ajoute une couche au graphique `ggplot()` grâce au symbole `+`. La couche en question précise le troisième élément indispensable de la grammaire des graphiques : l'objet `geom`étrique. Ici, les objets sont des `point`s. On le spécifie grâce à la fonction `geom_point()`.
...
## Quel graphique dans quelle situation ?
...
1. `Warning: Removed 2 rows containing non-finite values (stat_bin)`. Ce message indique, comme pour le premier nuage de points, que 2 individus du tableau `penguins` ont une masse corporelle inconnue (`NA`). Ces 2 individus (donc les deux lignes correspondantes), ont été ignorés pour produire ce graphique
...
Vous pouvez voir à quelle couleur correspond chacun de ces noms [dans ce document pdf](http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf).
...
#### À l'intérieur ou à l'extérieur de `aes()` ?
...
```{r}
ggplot(penguins, aes(x = body_mass_g)) +
  geom_histogram(fill = "steelblue", color = "black", binwidth = 200)
```
...
limites
ggplot(penguins, aes(x = body_mass_g)) +
  geom_histogram(fill = "steelblue", color = "black", breaks = limites)
```
...
#### `geom_rug` et `geom_density` {#sec-density}
...
#### Un mot sur la position de la fonction `aes()`
...
ggplot(data = penguins) +
  geom_histogram(
    aes(x = body_mass_g, y = after_stat(density)),
    fill = "steelblue",
    color = "black",
    bins = 25,
    alpha = 0.7
  ) +
  geom_rug(aes(x = body_mass_g), alpha = 0.3) +
  geom_density(aes(x = body_mass_g), color = "purple", linewidth = 2)
```

C'est plus long, mais c'est tout à fait correct et ça produit exactement le même résultat qu'auparavant.

### Les nuages de points et stripcharts {#sec-cloud}
...
```{r}
#| label: fig-cloud
#| fig-cap: "Nuage de points des masses corporelles des 3 espèces de manchots"
...
![Liste des symboles et codes correspondants pour les graphiques faisant apparaître des points. Pour les symboles 21 à 25, il sera possible de spécifier une couleur de remplissage `fill` et une couleur de contour `color`. Pour tous les autres symboles, les changements de couleurs se feront avec l'argument `color`.](images/pch.png){#fig-pch width=50%}
...
```{r}
#| label: fig-jitter
#| fig-cap: Trois exemples de stripchart
#| fig-width: 3
#| fig-subcap: 
#|   - "Pas de dispersion horizontale, pas de dispersion verticale"
#|   - "Faible dispersion horizontale, pas de dispersion verticale"
#|   - "Faible dispersion horizontale, forte dispersion verticale"
#| layout-ncol: 3
#| warning: false
...
#| fig-width: 5
#| label: fig-strip
#| fig-cap: "Un exemple de stripchart"
...
Notez que les figures @fig-barplot et @fig-barplotcol sont absolument identiques (à l'exception du titre de l'axe des ordonnées), mais qu'elles ont été créées à partir de 2 tableaux de données différents. En particulier, notez que :

- Le code qui génère la figure @fig-barplot utilise le jeu de données `panier`, et n'associe pas de variable à l'axe des ordonnées : dans la fonction `aes()`, seule la variable associée à `x` est précisée. C'est la fonction `geom_bar()` qui calcule automatiquement les abondances (ou fréquences) pour chaque catégorie de la variable `fruit`. La variable `count` est ainsi générée automatiquement et associée à `y`.
- Le code qui génère la figure @fig-barplotcol utilise le jeu de données `panier_counted`. Ici, c'est bien l'utilisateur qui associe la variable `nombre` à l'axe des `y` à l'intérieur de la fonction `aes()`. La fonction `geom_col()` a besoin de 2 variables (une variable catégorielle pour l'axe des `x` et une numérique pour l'axe des `y`) pour fonctionner.
...
- Si la variable catégorielle n'est pas pré-comptée dans le tableau de données {{< fa arrow-right >}} `geom_bar()`. La variable catégorielle est associée à l'esthétique `x` du graphique. On ne renseigne pas `y`.
- Si la variable catégorielle est pré-comptée dans le tableau de données {{< fa arrow-right >}} `geom_col()`. La variable catégorielle est associée à l'esthétique `x` du graphique. On associe explicitement les comptages à l'esthétique `y` du graphique.
:::
...
```{r}
#| label: fig-bpspecies
#| fig-cap: "Effectifs pour les 3 espèces de manchots étudiées"
...
```{r}
#| warning: false
#| label: fig-bpspecies-infreq
#| fig-cap: "Effectifs pour les 3 espèces de manchots étudiées, triés en ordre décroissant"
...
```{r}
ggplot(species_table, aes(x = fct_reorder(species, n, .desc = TRUE), y = n)) +
  geom_col()
```
...
ggplot(species_table, aes(y = fct_reorder(species, n, .desc = TRUE), x = n)) +
...
### Éviter à tout prix les diagrammes circulaires {#sec-pie}
...
```{r}
#| label: fig-pie
#| fig-cap: "Répartition des effectifs par espèce et par sexe"
#| echo: false

penguins |>
  count(species, sex) |>
  mutate(cat = paste(species, sex, sep = " - ")) |>
  ggplot(aes(x = factor(1), y = n, fill = factor(cat))) +
...
Il est difficile (voir impossible) de répondre précisément à ces questions avec le diagramme circulaire de la @fig-pie, alors qu'il est très simple d'obtenir des réponses précises avec un diagramme bâtons tel que présenté à la @fig-bpspecies-sex ci-dessous (vérifiez-le !) :

```{r}
#| echo: false
#| label: fig-bpspecies-sex
#| fig-cap: "Répartition des effectifs par espèce et par sexe"
...
penguins |>
  ...
  mutate(cat = paste(species, sex, sep = " - ")) |>
  ggplot(aes(
    y = fct_reorder(cat, n, .desc = TRUE),
    x = n,
    fill = fct_reorder(cat, n, .desc = TRUE)
  )) +
  geom_col(color = "black", show.legend = FALSE, alpha = 0.5) +
  scale_fill_brewer() +
  labs(x = "Effectif", y = "Catégorie") +
  theme_bw()
```


## Deux variables numériques {#sec-deuxnum}
...
#### Autres caractéristiques esthétiques
...
L'argument `se = FALSE` de la fonction `geom_smooth()` permet de ne pas afficher l'intervalle d'incertitude de la régression linéaire. Ici, j'ai associé la couleur de remplissage des points et la couleur des droites de régression aux espèces (donc à l'intérieur de `aes()`, soit dans `ggplot()` soit dans `geom_smooth()`), et j'ai fixé pour tous les points, le choix du type de symbole (`shape = 21`, voir @fig-pch), la couleur de contour (`color = "black"`) et la transparence `alpha = 0.6`.
...
![Exemple de carte produite dans `R` avec les packages `sf`, `marmap` et `ggplot2`.](images/map.png){#fig-map}
...
### Diagrammes bâtons empilés {#sec-empil}
...
### Diagrammes bâtons juxtaposés {#sec-juxta}
...
### Mosaïc plots {#sec-mosa}


Les graphiques en mosaïque sont une alternative aux diagrammes bâtons en tous genre. Ils permettent de visualiser à la fois les effectifs et de comparer les proportions. La difficulté de ce genre de graphique est qu'il n'existe pas d'objet géométrique permettant de les représenter simplement dans le package `ggplot2`. Le package `ggmosaic` de @R-ggmosaic est toutefois entièrement dédié à ce type de graphique. Pour installer ce packages, il faut procéder un peu différemment de d'habitude car nous devons installer sa version de développement (c'est une version plus récente que celle qui est disponible sur les serveurs officiels du CRAN). Pour cela, nous allons utiliser le package `remotes`. Si vous ne l'avez pas encore installé, faites-le maintenant :
...
Attention, il est possible que `R` vous propose d'installer d'autres packages dont `remotes` dépend. Si c'est le cas, vous devrez l'indiquer dans la console quand `R` vous posera la question en tapant la réponse appropriée (par exemple, le chiffre `1` pour installer toutes les dependances conseillées). Il est également possible que l'instalation de certaines de ces dependances ne soit pas possible tant que `R` n'aura pas été redémarré. Si c'est le cas, redémarrez `R` (en cliquant dans le menu `Session > Restart R` dans RStudio). puis relancez la commande `install.packages("remotes")` jusqu'à ce que l'installation se fasse sans erreur. Vous pourrez alors installer la version de développement du package `ggmosaic` ainsi :
...
Si vous avez dû redémarrer `R` pour installer le package `remotes`, n'oubliez pas de recharger l'ensemble des packages nécessaires pour ce chapitre :
...
Ajoutez-y le package `ggmosaic` nouvellement installé :
...
## Une variable de chaque type {#sec-oneeach}
...
2. les stripcharts
3. les boîtes à moustaches. Nous donnerons ici un simple. La signification de tous les éléments de ces graphiques sera fournie à la @sec-reframe
...
### Histogrammes "facettés" {#sec-factorhisto}
...
ggplot(penguins, aes(x = bill_depth_mm)) +
  geom_histogram(fill = "steelblue", color = "black", alpha = 0.6, bins = 20) +
...
ggplot(penguins, aes(x = bill_depth_mm, fill = species)) +
  geom_histogram(show.legend = FALSE, color = "black", alpha = 0.6, bins = 20) +
  facet_wrap(~species, ncol = 1, scales = "free_y")
```

Les Gentoo, qui ont pourtant des masses corporelles supérieures à celle des deux autres espèces (voir @fig-cloud de la @sec-cloud), ont visiblement des becs moins épais (entre 12 et 17 mm) que les deux autres espèces (entre 16 et 22 mm). 
...
### Les stripcharts {#sec-factorstrip}

Nous avons déjà abordé ce type de graphique dans la @sec-cloud. Contrairement à la situation où nous n'avions qu'une variable numérique et où nous devions fixer `x = ""` pour que toutes les observations se placent au même niveau de l'axe des abscisses (voir @fig-strip), nous allons ici associer la variable catégorielle à l'axe des `x`. La variable numérique sera quant-à-elle toujours associée à l'axe des ordonnées :
...
### Les boîtes à moustaches ou boxplots {#sec-boxplot}
...
Dans la forme, ça ressemble un à un stripchart (comparez par exemple avec la syntaxe et les résultats obtenus à la @fig-facstrip). Néanmoins, ici, au lieu de visualiser tous les points du jeu de données, seules quelques valeurs caractéristiques sont utilisées pour construire le boîte à moustache de chaque espèce. Les différents éléments d'un boxplot, sont les suivants :
...
Lorsque les boîtes ont une forme à peu près symétrique de part et d'autre de la médiane (c'est le cas pour notre exemple), cela signifie qu'un histogramme des mêmes données serait symétrique également (on peut le vérifier avec les histogrammes de la @sec-factorhisto).
...
La @fig-comboxplot illustre ces 3 possibilités de visualisation de la distribution d'une variable numérique (ici, la distribution des masses corporelles des manchots Adélie) :

```{r}
#| label: fig-comboxplot
#| fig-cap: "Trois façons de visualiser la distribution des masses des manchots Adélie"
#| message: false
#| warning: false
#| echo: false
...
pl1 <- penguins |>
  ...
  ggplot(aes(x = "Adelie", y = body_mass_g)) +
  geom_jitter(width = 0.2, height = 0, alpha = 0.6)

pl2 <- penguins |>
  filter(species == "Adelie") |>
  ggplot(aes(x = body_mass_g)) +
  geom_histogram(bins = 15, color = "black", fill = "grey85") +
  coord_flip()
...
penguins |>
  ...
  geom_point(alpha = 0.6) +
  scale_shape_manual(labels = c("Femelles", "Mâles"), values = c(21, 24)) +
  scale_fill_brewer(
    palette = "Accent",
...
```{r}
#| label: fig-multivar
#| fig-cap: "Relation entre la morphologie du bec, la masse et le sexe chez trois espèces de manchots de l'archipel Palmer"
#| echo: false

penguins |>
  filter(!is.na(sex)) |>
  mutate(species = str_replace_all(species, "Adelie", "Adélie")) |>
  ggplot(aes(
    x = bill_length_mm,
    y = bill_depth_mm,
    fill = sex,
...
### Deux variables numériques et un facteur {#sec-simpson}
...
En statistiques ce phénomène (observer une relation inverse lorsque plusieurs groupes sont combinés) s'appelle le paradoxe de Simpson, et je vous encourage à consulter [la page wikipédia qui y est consacrée](https://fr.wikipedia.org/wiki/Paradoxe_de_Simpson).
...
### Les légendes ou labels {#sec-labels}
...
ggplot(
  ...
  geom_point(shape = 21, color = "black", alpha = 0.6) +
  labs(
    title = "Forme du bec de 3 espèces de manchots et relation avec leur masse",
...
```{r}
#| label: fig-label
#| fig-cap: "Une figure correctement légendée"
...
Nous allons néanmoins examiner quelques possibilités, à commencer par la façon de procéder pour modifier les couleurs choisies par défaut par `ggplot2`. Reprenons la figure @fig-label, et changeons le gradient de couleur proposé par défaut par `R`. Il est possible de modifier ces couleurs de plusieurs façons :
...
Par exemple, il est possible d'utiliser la palette `viridis`. Selon [ses auteurs](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html) : 
...
p1 <- pp +
  scale_fill_viridis_c(option = "A") +
  ggtitle('option = "A" (ou option = "magma")')
p2 <- pp +
  scale_fill_viridis_c(option = "B") +
  ggtitle('option = "B" (ou option = "inferno")')
p3 <- pp +
  scale_fill_viridis_c(option = "C") +
  ggtitle('option = "C" (ou option = "plasma")')
p4 <- pp +
  scale_fill_viridis_c(option = "D") +
  ggtitle('option = "D" (ou option = "viridis")')
p5 <- pp +
  scale_fill_viridis_c(option = "E") +
  ggtitle('option = "E" (ou option = "cividis")')
...
```{r fig-gill-sans, dev='ragg_png', fig.ext='png'}
ggplot(
  penguins,
  aes(x = bill_length_mm, y = bill_depth_mm, fill = species, size = body_mass_g)
) +
  geom_point(shape = 21, color = "black", alpha = 0.6) +
  labs(
    title = "Forme du bec de 3 espèces de manchots et relation avec leur masse",
...
```{r fig-gill-sans-size, dev='ragg_png', fig.ext='png'}
ggplot(
  penguins,
  aes(x = bill_length_mm, y = bill_depth_mm, fill = species, size = body_mass_g)
) +
...
Dernière chose concernant les couleurs : un choix de fonction `scale_XXX_XXX()` inapproprié est une cause d'erreur très fréquente ! Par exemple, pour la première figure de la partie consacrée au paradoxe de Simpson (@sec-simpson), la couleur des points et des lignes n'est pas spécifiée avec `fill` mais avec `color`. C'est donc bien une fonction qui commence par `scale_color_` qu'il faut utiliser : 
...
ggplot(
  ...
  geom_point(shape = 21, color = "black", alpha = 0.6) +
  labs(
    title = "Forme du bec des manchots et relation avec leur masse",
    subtitle = "Les manchots les plus lourds ont des becs longs et fins",
    x = "Longueur du bec (mm)",
    y = "Épaisseur du bec (mm)",
    caption = "Source :  package 'palmerpenguins'",
...
#| echo: false
#| warning: false
#| fig-cap: "Prix de 3000 diamants en fonction de leur taille en carats et de leur clarté."
ggplot(data = diams, mapping = aes(x = carat, y = price)) +
  geom_point(alpha = 0.5, shape = 4, aes(color = clarity)) +
  labs(x = "Carat", y = "Prix ($US)", color = "Clarté") +
  theme_bw()
```
...
small_flights <- flights |>
  filter(!is.na(arr_delay), distance < 3000) |>
  sample_n(1000)
```
