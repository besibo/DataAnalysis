...
Les commandes permettant de produire ce tableau seront expliquées dans le @sec-wrangling. On peut cependant constater ici que ce tableau contient 35 lignes et 3 colonnes. Il s'agit bien d'un "tableau rangé" parfaitement adapté pour faire des statistiques et des visualisations graphiques, mais son format n'est pas terrible si notre objectif est de le faire figurer dans un rapport. La solution : utiliser `pivot_wider()` :
...
La nouvelle fenêtre qui s'ouvre est un "assistant d'importation" (@fig-import2).
