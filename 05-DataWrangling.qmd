...
adelie_light <- penguins |>
  filter(species == "Adelie", body_mass_g <= 3700)
...
penguins |>
  select(species:island, b_length = bill_length_mm, flipper = flipper_length_mm)
```
...
penguins |>
  rename(b_length = bill_length_mm, flipper = flipper_length_mm)
```
...
penguins |>
  mutate(ratio = bill_length_mm / bill_depth_mm, mass_kg = body_mass_g / 1000)
```
...
library(readxl)
dauphin <- read_excel("data/dauphin.xls", na = "*", skip = 9) |> # Importer, puis
  rename(
    ID = `N°`, # Raccourcir les noms, puis
...
dauphin_clean <- dauphin |>
  ...
  mutate(
    Sexe = factor(Sexe), # Transformation de Sexe en facteur
    Organe = factor(Organe), # Transformation d'Organe en facteur
    Statut = factor(
      Statut, # Transformation de Statut en facteur
...
dauphin |>
  mutate(Sexe = fct_recode(Sexe, "Femelle" = "f", "Mâle" = "m"))
```
...
dauphin |>
  ...
  labs(
    x = "Organe",
    fill = "Sexe",
    y = "Concentration en cuivre (µg/g de poids sec)"
  ) +
  theme_bw()
```
...
dauphin_clean <- dauphin |>
  ...
  mutate(
    Sexe = fct_recode(
      Sexe, # Changement des modalités du facteur Sexe, puis
      "Femelle" = "f",
      "Mâle" = "m"
    ),
    Organe = fct_rev(Organe), # Inversion des modalités du facteur Organe
    Statut = fct_relevel(
      Statut, # Ré-agencement de l'ordre des modalités du facteur Statut
      "imm",
      "mat",
      "pnl",
      "l",
...
dauphin <- read_excel("data/dauphin.xls", na = "*", skip = 9) |> # Importer, puis
  ...
  mutate_if(
    is.character, # 'Factoriser' les variables <chr>, puis
    as.factor
  ) |>
  mutate(
    Sexe = fct_recode(
      Sexe, # Modifier les modalités des facteurs
      "Female" = "f",
      "Male" = "m"
    ),
...
penguins |>
  summarise(
    moy_flip = mean(flipper_length_mm, na.rm = TRUE),
...
penguins |>
  filter(!is.na(sex)) |>
  summarise(moyenne = mean(bill_length_mm, na.rm = TRUE), .by = c(species, sex))
```
...
penguins |>
  ...
  pivot_wider(names_from = sex, values_from = moyenne)
```
...
```{r}
penguins |>
  summarise(effectif = n(), .by = species)
```
...
weather |>
  ...
  labs(x = "Mois", y = "Vitesse du vent (mph)")
```
...
```{r}
penguins |>
  filter(body_mass_g > quantile(body_mass_g, 0.9, na.rm = TRUE), .by = species)
```
...
Pour bien enfoncer le clou, voici un autre exemple, impliquant la fonction `mutate()`. Imaginez que nous ayons besoin d'identifier, toujours dans le tableau `penguins`, les individus de chaque espèce dont la longueur du bec est supérieure à la moyenne de l'espèce. Il nous faut donc calculer, pour chaque espèce, la moyenne des longueurs de bec, puis créer une nouvelle colonne de vrais/faux pour chaque individu de chaque espèce : ceux dont le bec est supérieur à la moyenne auront `TRUE` dans cette nouvelle colonne, les autres auront `FALSE`. Voilà comment nous pourrions procéder grâce à l'argument `.by` :

```{r}
becs <- penguins |>
  mutate(
    long_bill = bill_length_mm >= mean(bill_length_mm, na.rm = TRUE),
    .by = species
  )
...
penguins |>
  ...
  ggplot(aes(
    x = species,
    y = body_mass_g,
    color = bill_length_mm >= mean(bill_length_mm, na.rm = TRUE)
  )) +
  geom_jitter(height = 0, width = 0.2) +
  facet_wrap(~sex, ncol = 2) +
  labs(x = "", y = "Masse corporelle (g)", color = "Taille du bec") +
...
inner_late |>
  ...
  geom_smooth() +
  labs(
    x = "Âge des avions (en années)",
    y = "Retard à l'arrivée des vols (en minutes)"
  ) +
  theme_bw()
```
...
a <- tibble(x = 1:3, y = c(2, 4, 6), z = c(TRUE, FALSE, FALSE))

b <- tibble(r = 10:8, s = rnorm(3))
