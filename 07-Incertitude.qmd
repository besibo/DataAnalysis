...
## La notion d'incertitude {#sec-incertitude}
...
penguins |>
  filter(!is.na(sex)) |>
  summarise(moyenne = mean(body_mass_g, na.rm = TRUE), .by = c(species, sex))
```
...
penguins |>
  filter(!is.na(sex)) |>
  reframe(mean_se(body_mass_g), .by = c(species, sex))
```
...
penguins |>
  filter(!is.na(sex)) |>
  reframe(mean_se(body_mass_g, mult = 1.96), .by = c(species, sex))
```
...
penguins |>
  filter(!is.na(sex)) |>
  reframe(mean_cl_normal(body_mass_g), .by = c(species, sex))
```

Comme dans les tableaux précédents, 3 nouvelles colonnes ont été crées :

- `y` contient toujours la moyenne des masses corporelles pour chaque espèce et chaque sexe
...
masses_se_bornes <- penguins |>
  filter(!is.na(sex)) |>
  reframe(mean_se(body_mass_g), .by = c(species, sex)) |>
  rename(moyenne = y, moyenne_moins_se = ymin, moyenne_plus_se = ymax)
...
masses_ci_approx <- penguins |>
  filter(!is.na(sex)) |>
  reframe(mean_se(body_mass_g, mult = 1.96), .by = c(species, sex)) |>
  rename(moyenne = y, ci_borne_inf = ymin, ci_borne_sup = ymax)
